...
function buildNotification(input: NotificationInput, createdAt: string): AppNotification {
  const id = adminDb.collection("pg_notifications").doc().id;
...
export async function createNotifications(
  ...
  notifications.forEach((notification) => {
    const ref = adminDb.collection("pg_notifications").doc(notification.id);
...
export async function markNotificationAsRead(
  ...
  const ref = adminDb.collection("pg_notifications").doc(notificationId);
