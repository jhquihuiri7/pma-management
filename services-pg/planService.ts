...
import { getAuthenticatedDrive } from "@/lib/drive-pg";
...
export async function createPlan(
  ...
  const planRef = adminDb.collection("pg_projects").doc();
...
export async function getPlanById(planId: string): Promise<Plan | null> {
  const doc = await adminDb.collection("pg_projects").doc(planId).get();
...
export async function updatePlan(
  ...
  await adminDb.collection("pg_projects").doc(planId).update(updateData);
...
export async function deletePlan(
  ...
  const [assignments, evidences, planItems, periodCompliance, monthlyGeneration, notifications] =
    await Promise.all([
      adminDb.collection("pg_project_assignments").where("planId", "==", planId).get(),
      adminDb.collection("pg_evidences").where("planId", "==", planId).get(),
      adminDb.collection("pg_projectItems").where("planId", "==", planId).get(),
      adminDb.collection("pg_project_period_compliance").where("planId", "==", planId).get(),
      adminDb.collection("pg_project_monthly_generation").where("planId", "==", planId).get(),
      adminDb.collection("pg_notifications").where("planId", "==", planId).get(),
    ]);
  ...
  const allRefs = [
    ...
    adminDb.collection("pg_projects").doc(planId),
  ];
...
export async function assignUserToPlan(
  ...
  const existing = await adminDb
    .collection("pg_project_assignments")
    .where("userId", "==", userId)
    .where("planId", "==", planId)
    .limit(1)
    .get();
  ...
  const ref = adminDb.collection("pg_project_assignments").doc();
...
export async function getPlansForReporter(userId: string): Promise<Plan[]> {
  ...
  const itemSnap = await adminDb.collection("pg_projectItems").get();
  ...
  const refs = Array.from(planIdSet).map((pid) =>
    adminDb.collection("pg_projects").doc(pid)
  );
...
export async function getPlansForViewer(userId: string): Promise<Plan[]> {
  ...
  const refs = planIds.map((pid) => adminDb.collection("pg_projects").doc(pid));
  const snapshots = await adminDb.getAll(...refs);
  return snapshots
    .filter((doc) => doc.exists)
    .map((doc) => {
