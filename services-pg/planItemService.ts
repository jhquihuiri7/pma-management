...
export async function createPlanItem(
  ...
  const ref = adminDb.collection("pg_projectItems").doc();
...
export async function assignReporterToItem(
  ...
  const ref = adminDb.collection("pg_projectItems").doc(itemId);
  ...
  if (existing.empty) {
    const assignRef = adminDb.collection("pg_project_assignments").doc();
    await assignRef.set({
      id: assignRef.id,
      userId,
      planId: item.planId,
...
export async function deletePlanItem(
  ...
  const doc = await adminDb.collection("pg_projectItems").doc(itemId).get();
  if (!doc.exists) throw new Error("Item not found");
