...
export async function createEvidence(
  ...
  const ref = adminDb.collection("pg_evidences").doc();
...
export async function updateEvidenceValidation(
  ...
  const ref = adminDb.collection("pg_evidences").doc(evidenceId);
  const doc = await ref.get();
  if (!doc.exists) throw new Error("Evidence not found");

  const current = doc.data() as Evidence;

  const planDoc = await adminDb.collection("pg_projects").doc(current.planId).get();
...
export async function deleteEvidence(
  ...
  const doc = await adminDb.collection("pg_evidences").doc(evidenceId).get();
  if (!doc.exists) throw new Error("Evidence not found");
  ...
  await adminDb.collection("pg_evidences").doc(evidenceId).delete();
}
