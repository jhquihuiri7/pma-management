...
import { sendEmailFromAdmin, buildInvitationEmail, buildPasswordRecoveryEmail } from "@/lib/gmail-rgdp";
...
async function createManagedUser(
  ...
  const userRef = adminDb.collection("rgdp_users").doc();
  ...
  const setPasswordLink = `${appUrl}/rgdp/set-password?token=${token}`;
...
export async function resendInvitation(
  ...
  const doc = await adminDb.collection("rgdp_users").doc(userId).get();
  ...
  await adminDb.collection("rgdp_users").doc(userId).update({
    passwordSetToken: token,
    passwordSetTokenExpiry: tokenExpiry,
    emailSent: true,
  });
...
export async function setUserPassword(
  ...
  await adminDb.collection("rgdp_users").doc(info.userId).update({
...
export async function generatePasswordRecoveryToken(email: string): Promise<void> {
  ...
  const appUrl = process.env.NEXTAUTH_URL ?? "http://localhost:3000";
  const resetLink = `${appUrl}/rgdp/reset-password?token=${token}`;
...
export async function deleteReporter(
  ...
  batch.delete(adminDb.collection("rgdp_users").doc(userId));
  await batch.commit();
}
