...
export async function createEvidence(
  ...
  const ref = adminDb.collection("rgdp_evidences").doc();
...
export async function updateEvidenceValidation(
  ...
  const ref = adminDb.collection("rgdp_evidences").doc(evidenceId);
...
export async function deleteEvidence(
  ...
  const doc = await adminDb.collection("rgdp_evidences").doc(evidenceId).get();
  if (!doc.exists) throw new Error("Evidence not found");
