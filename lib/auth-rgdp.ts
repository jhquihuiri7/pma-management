...
import { createRootFolder } from "./drive-rgdp";
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const usersRef = adminDb.collection("rgdp_users");
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      if (account?.provider === "google") {
        const adminRef = adminDb.collection("rgdp_admins").doc(user.id);
        ...
        if (!adminDoc.exists) {
          ...
          await adminDb.collection("rgdp_users").doc(user.id).set({
            id: user.id,
            name: user.name,
            email: user.email,
            role: "ADMIN",
