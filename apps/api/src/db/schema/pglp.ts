...
export const pglpPlans = pgTable(
  "pglp_plans",
  {
  ...
  (t) => ({
    adminIdx: index("pglp_plans_admin_idx").on(t.adminId),
  })
);

export const pglpPlanItems = pgTable(
  "pglp_plan_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    planId: uuid("plan_id")
      .notNull()
      .references(() => pglpPlans.id, { onDelete: "cascade" }),
  ...
  (t) => ({
    planIdx: index("pglp_plan_items_plan_idx").on(t.planId),
  })
);

export const pglpPlanAssignments = pgTable(
  "pglp_plan_assignments",
  {
    planId: uuid("plan_id")
      .notNull()
      .references(() => pglpPlans.id, { onDelete: "cascade" }),
...
export const pglpItemAssignments = pgTable(
  "pglp_item_assignments",
  {
    planItemId: uuid("plan_item_id")
      .notNull()
      .references(() => pglpPlanItems.id, { onDelete: "cascade" }),
    userId: uuid("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
...
export const pglpEvidences = pgTable(
  "pglp_evidences",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    planId: uuid("plan_id")
      .notNull()
      .references(() => pglpPlans.id, { onDelete: "cascade" }),
    planItemId: uuid("plan_item_id").references(() => pglpPlanItems.id, { onDelete: "cascade" }),
  ...
  (t) => ({
    planIdx: index("pglp_evidences_plan_idx").on(t.planId),
    itemIdx: index("pglp_evidences_item_idx").on(t.planItemId),
  })
);

export const pglpFindings = pgTable(
  "pglp_findings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    planId: uuid("plan_id")
  ...
  (t) => ({
    planIdx: index("pglp_findings_plan_idx").on(t.planId),
  })
);

export const pglpPeriodCompliance = pgTable(
  "pglp_period_compliance",
  {
    planItemId: uuid("plan_item_id")
      .notNull()
      .references(() => pglpPlanItems.id, { onDelete: "cascade" }),
...
export const pglpMonthlyGenerations = pgTable(
  "pglp_monthly_generations",
  {
    planItemId: uuid("plan_item_id")
      .notNull()
      .references(() => pglpPlanItems.id, { onDelete: "cascade" }),
...
export const pglpNotifications = pgTable(
  "pglp_notifications",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id")
    ...
    planId: uuid("plan_id").references(() => pglpPlans.id, { onDelete: "cascade" }),
    planItemId: uuid("plan_item_id").references(() => pglpPlanItems.id, { onDelete: "cascade" }),
    evidenceId: uuid("evidence_id").references(() => pglpEvidences.id, { onDelete: "cascade" }),
  ...
  (t) => ({
    userIdx: index("pglp_notifications_user_idx").on(t.userId),
  })
);

export const pglpFormats = pgTable("pglp_formats", {
  id: uuid("id").primaryKey().defaultRandom(),
  adminId: uuid("admin_id")
    .notNull()
    .references(() => admins.id, { onDelete: "cascade" }),
