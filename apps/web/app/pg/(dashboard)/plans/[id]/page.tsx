...
export default function PlanDetailPage() {
  ...
  const loadPlan = useCallback(async () => {
    const res = await apiFetch(`/pg/api/plans/${id}`);
    if (res.ok) {
  ...
  const loadItems = useCallback(async () => {
    const res = await apiFetch(`/pg/api/plans/${id}/items`);
    if (res.ok) setPlanItems(await res.json());
  }, [id]);

  const loadMonthlyGeneration = useCallback(async () => {
    const res = await apiFetch(`/pg/api/plans/${id}/monthly-generation`);
  ...
  async function handleDeletePlan() {
    ...
    try {
      const res = await apiFetch(`/pg/api/plans/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  async function handleAssignViewer(viewerId: string) {
    const res = await apiFetch(`/pg/api/plans/${id}/assign`, {
      method: "POST",
  ...
  async function handleValidationChange(
    ...
    const res = await apiFetch(`/pg/api/evidences?id=${evidenceId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function handleAddItem(e: React.FormEvent) {
    ...
    try {
      if (editingItem) {
        const res = await apiFetch(`/pg/api/plans/${id}/items/${editingItem.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function handleBulkSubmit() {
    ...
    try {
      const res = await apiFetch(`/pg/api/plans/${id}/items/bulk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleAssignToItem(userId: string, category: ItemAssignmentCategory) {
    ...
    const res = await apiFetch(`/pg/api/plans/${id}/items/${selectedItem.id}/assign`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (res.ok) {
      ...
      const updated = await apiFetch(`/pg/api/plans/${id}/items`);
      if (updated.ok) {
  ...
  async function syncItemAssignments(
    ...
    const responses = await Promise.all([
      ...toRemove.map((userId) =>
        apiFetch(`/pg/api/plans/${id}/items/${itemId}/assign`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId }),
        })
  ...
  async function handleCalUploadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await apiFetch("/pg/api/upload", { method: "POST", body: formData });
  ...
  async function handleDownloadPeriod(pi: PlanItem, periodKey: string) {
    ...
    try {
      ...
      const res = await apiFetch(`/pg/api/download/item-period?${params}`);
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  async function handleDeleteItem(itemId: string) {
    ...
    const res = await apiFetch(`/pg/api/plans/${id}/items/${itemId}`, {
      method: "DELETE",
    });
  ...
  async function handleSaveObservation() {
    if (!obsItem) return;
    setSavingObs(true);
    const res = await apiFetch(`/pg/api/plans/${id}/items/${obsItem.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
