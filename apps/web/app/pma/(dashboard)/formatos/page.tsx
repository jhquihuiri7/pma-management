...
import { api, ApiError, apiErrorMessage, requireOkReceipt, requirePersistedAsset } from "@/lib/api-client";
...
export default function FormatosPage() {
  ...
  async function fetchFormats() {
    setLoading(true);
    try {
      setFormats(await api.get<Format[]>("/pma/api/formats"));
    } catch (error) {
  ...
  async function handleUpload(functionality: FormatFunctionality, file: File) {
    ...
    try {
      ...
      const saved = requirePersistedAsset<Format>(
        await api.upload<unknown>("/pma/api/formats", formData, { timeoutMs: 60_000 }),
  ...
  async function handleDelete(format: Format) {
    ...
    try {
      requireOkReceipt(
        await api.delete<unknown>(`/pma/api/formats/${format.id}`),
