...
const adminLinks = [
  { href: "/pg/dashboard", label: "Panel Principal", icon: LayoutDashboard },
  { href: "/pg/plans", label: "Proyectos", icon: FileText },
  { href: "/pg/users", label: "Usuarios", icon: Users },
];

const reporterLinks = [
  { href: "/pg/dashboard", label: "Panel Principal", icon: LayoutDashboard },
  { href: "/pg/plans", label: "Mis Proyectos", icon: FileText },
];
...
export default function SidebarPG() {
  ...
  return (
    <aside className="w-64 h-screen bg-white border-r border-slate-200 flex flex-col fixed left-0 top-0">
      <div className="p-6">
        <h1 className="text-lg font-bold text-slate-900">Plan Galapagos</h1>
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        {links.map((link) => {
          ...
          const isActive =
            pathname === link.href ||
            (link.href !== "/pg/dashboard" && pathname.startsWith(link.href));
        ...
        {role === "ADMIN" && (
          <>
            ...
            <a
              href="/pg/formatos"
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                pathname === "/pg/formatos" || pathname.startsWith("/pg/formatos")
                  ? "bg-slate-100 text-slate-900"
                  : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
              }`}
            >
