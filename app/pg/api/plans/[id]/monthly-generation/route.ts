...
import { getPlanById, isUserAssignedToPlan } from "@/services-pg/planService";
...
export async function GET(
  ...
  const snapshot = await adminDb
    .collection("pg_project_monthly_generation")
...
export async function POST(
  ...
  const generationCollection = adminDb.collection("pg_project_monthly_generation");
  const recordId = `${planItemId}_${periodKey}`;
  const recordRef = generationCollection.doc(recordId);
  const itemRef = adminDb.collection("pg_projectItems").doc(planItemId);
